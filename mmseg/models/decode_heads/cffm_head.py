...
from .cffm_module.cffm_transformer import BasicLayer3d3   # 只用其decoder
...
class CFFMHead_clips_resize1_8(BaseDecodeHead_clips_flow):
    ...
    def __init__(self, feature_strides, **kwargs):
        ...
        self.decoder_focal=BasicLayer3d3(dim=embedding_dim,
            ...
            #    focal_l_clips=[1,2,3],       # 只2*2的window
        ...
        self.mask_size = 7          # mask的局部注意力窗口大小，dsec:8
        # if self.training:
    ...
    def forward(self, inputs, batch_size=None, num_clips=None, imgs=None):
        ...
        # if batch_size != 2 and batch_size != 8:
        if num_clips!=4:
        ...
        h2=int(h/2)
        w2=int(w/2)     # 先把h,w = 120下采样到60，60..符合上面的input_resolution；也就是原分辨率的1/8
        _c = resize(_c, size=(h2,w2),mode='bilinear',align_corners=False)  # 这个_一般表示临时变量。
        # b d c h w
        _c_further=_c.reshape(batch_size, num_clips, -1, h2, w2)

        # nb,_,nh,nw = c2.shape
        q_target = _c_further[:,-1:]    # 2, 1, 256, 60, 60  这个是target的综合特征，用来计算跨帧注意力的
        kv_ref1 = _c_further[:, 0:1, :, :, :]   # 这个是ref的综合特征，用来计算精化ref
        ...
        # mask = create_attention_masks(B=batch_size,H=h2,W=w2,mask_size=self.mask_size)
        ...
        final_out_1layer = self.decoder_focal(refined_features)
