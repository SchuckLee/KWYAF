...
class WindowAttention3d3(nn.Module):
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads, 
        ...
        self.proj = nn.Linear(dim, dim) # 得到了attention后再proj
    ...
    def forward(self, x_all, batch_size=None, num_clips=None):
        ...
        B0, nH, nW, C = x.shape         # 已更改！现在是B H W C 即 2 55 60 256
        ...
        (q_windows, k_windows, v_windows) = map(    # 对q k v划分window，Returns:  windows: (num_windows*B, window_size, window_size, C)
        ...
        if self.expand_size > 0 and self.focal_level > 0:
            ...
            k_rolled = torch.cat((k_windows, k_rolled), 2)  # 把直接window化 和 先平移再window化的k v 拼接起来；
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level-1):
                stride = 2**k
                x_window_pooled = x_all[0][k+1]  # B0, nWh, nWw, C  这里的x_window_pooled就是target pooled feature
                ...
                qkv_pooled = x_window_pooled.unsqueeze(0).repeat(3, 1, 1, 1, 1)  # 标记，qkv
            ...
            for k in range(len(self.focal_l_clips)):
                ...
                x_window_pooled = x_all[k+1]    # x_all[1 2 3]就是reference pooled features
                nWh, nWw = x_window_pooled.shape[1:3] 
        ...
        N = k_all.shape[-2] # 这个N，也就是所有level下的token总数（k,v的）

        q_windows = q_windows * self.scale  # 之前对target feat分了window后的q
        ...
        # q: torch.Size([192, 8, 49, 32]) k: torch.Size([192, 8, 289, 32])
        # attn torch.Size([192, 8, 49, 289]) v: torch.Size([192, 8, 289, 32])
        x = self.proj(x)
        x = self.proj_drop(x)
        # print(x.shape)
...
class CffmTransformerBlock3d3(nn.Module):
    ...
    def forward(self, x):
        ...
        B0, D0, H0, W0, C = x.shape  # D0是num_clips=4
        shortcut = x    # 验证过，不用clone
        ...
        if self.shift_size > 0: # shift_size=0,不必移动特征图
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        else:
            shifted_x = x
        shifted_x=shifted_x.view(B0,D0,H,W,C)
        x_windows_all = [shifted_x[:,-1]]       # 标记，获得后一帧，作为q
        x_windows_all_clips=[]  # [0]是x_windows_all，其中[0][0]是segformer提取的target feat，[0][1]是x_windows_pooled；
        if self.focal_level > 1 and self.pool_method != "none": 
            # if we add coarser granularity and the pool method is not none
            for k in range(self.focal_level-1):   # 执行1次，k=0  
                window_size_glo = math.floor(self.window_size_glo / (2 ** k))   # 全局窗口global,不变
                x_level_k = shifted_x[:,-1] # 标记，获得后一帧
            ...
            for k in range(len(self.focal_l_clips)):  # 3次，代表3个ref帧   
                
                window_size_glo = self.window_size_glo
                x_level_k = shifted_x[:,k]  # 标记，这里作为k v。因为k遍历次数为定义好的focal_l_clips长度，故应获得3个ref的特征  
                x_windows_noreshape = window_partition_noreshape(x_level_k.contiguous(), window_size_glo) # B0, nw, nw, window_size, window_size, C    
                x_windows_all_clips += [x_windows_noreshape]   # 第一个是query，后三个是ref， k v
        ...
        """把x和之前的shortcut的后一帧shortcut[:,-1]进行加和得到x'；最后再把x'和shortcut的前面的ref帧[shortcut[:,:-1]再进行concat。"""
