...
def generate_input_representation(events, event_representation, shape, nr_temporal_bins=5, separate_pol=True):
    ...
    if event_representation == 'histogram':
        ...
        return generate_single_channel_event_histogram(events, shape)   # 单通道，阴性阳性的总数量
    elif event_representation == 'voxel_grid':  # 使用这个
        return generate_voxel_grid(events, shape, nr_temporal_bins, separate_pol)


def generate_single_channel_event_histogram(events, shape):
    """
    生成单通道的事件计数图，将阴性和阳性事件的数量相加。
    
    参数:
    events: N x 4 数组, 每行依次为 x, y, t, polarity，其中 polarity 取值为 {-1, 1}，x 和 y 对应图像坐标 u 和 v
    shape: 图像的形状（高度, 宽度）
    ...
    # 初始化计数矩阵
    event_count = np.zeros((height * width,), dtype="float32")
    
    # 将正负事件的计数累加到同一个矩阵中
    np.add.at(event_count, x[p == 1] + width * y[p == 1], 1)
    np.add.at(event_count, x[p == -1] + width * y[p == -1], 1)
    
    # 将计数矩阵转换为图像
    histogram = event_count.reshape((height, width))
