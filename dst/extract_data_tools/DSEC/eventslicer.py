...
class EventSlicer:
    ...
    def get_events(self, t_start_us: int, t_end_us: int, max_events_per_data: int = -1) -> Dict[str, np.ndarray]:
        ...
        if(t_start_ms<=0 ):
            print('t_start_ms小于0！当前为：',t_start_ms)
        t_start_ms_idx = self.ms2idx(t_start_ms)    # start小于0
    ...
    def get_final_event_fixed_num(self, t_end_us: int, nr_events: int = 100000) -> Dict[str, np.ndarray]:
        """Get events (p, x, y, t) with fixed number of events
        Parameters
        ----------
        t_end_us: end time in microseconds
        ...
        _, idx_end_offset = self.get_time_indices_offsets(time_array_conservative, t_end_us, t_end_us)
        t_end_us_idx = t_end_lower_ms_idx + idx_end_offset
        # t_start_us_idx = t_end_us_idx - nr_events * 4   # 变成了往前倒退4*10w个事件，也就是堆叠最后一帧。
        t_start_us_idx = t_end_us_idx - nr_events    # 变成了往前倒退10w个事件，也就是堆叠最后一帧。
        if t_start_us_idx < 0:
            t_start_us_idx = 0
    ...
    def ms2idx(self, time_ms: int) -> int:
        # assert time_ms >= 0
        if(time_ms < 0 ):
            return self.ms_to_idx[0]    # 如果小于0就恢复为0
