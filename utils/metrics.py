...
def semseg_compute_confusion(y_hat_lbl, y_lbl, num_classes, ignore_label):
    ...
    """
    label_pred.unsqueeze(1).shape torch.Size([32, 1, 440, 640])
    label.unsqueeze(1).shape torch.Size([32, 1, 440, 640])
    y_hat_lbl.shape torch.Size([32, 440, 640])
    mask.shape torch.Size([32, 440, 640])
    """
...
class MetricsSemseg:
    ...
    def get_metrics_summary(self):
        ...
        iou_per_class = iou_per_class.numpy()
        iou_list = iou_per_class.tolist()
        iou_per_class = str(iou_list)
        return out,iou_per_class